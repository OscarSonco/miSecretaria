...
@Composable private fun SettingsScreen(onBack: () -> Unit, activity: MainActivity) {
    ...
    Scaffold { p -> Column(Modifier.padding(p).padding(16.dp).fillMaxSize().verticalScroll(rememberScrollState())) {
        ...
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) { TextButton(onClick = { activity.save("ScoSecretaria_ReporteV1.3_${WalletNotificationStore.timestampForFile()}.txt") }) { Text("Guardar Historial") }; TextButton(onClick = { activity.share("ScoSecretaria_ReporteV1.3.txt", WalletNotificationStore.exportText()) }) { Text("Compartir Historial") } }
        Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) { TextButton(onClick = { activity.save("ScoSecretariaV1.3.log") }) { Text("Guardar Log") }; TextButton(onClick = { activity.share("ScoSecretariaV1.3.log", ScoSecretariaLogger.read(context)) }) { Text("Compartir Log") } }
